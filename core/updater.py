...
# tools/embed_version.py. This eliminates the runtime file-lookup
# regression that's been plaguing us through v3.1.2 / v3.1.3 / v3.1.5.
# core/_version.py is generated by build.bat BEFORE PyInstaller runs.
try:
    from ._version import VERSION as _EMBEDDED_VERSION
except Exception:
...
def get_current_version() -> str:
    """v3.1.6 — prefer the build-baked _EMBEDDED_VERSION constant. The
    old file-lookup path is kept as a secondary signal (for dev runs
    that haven't generated _version.py yet) but is no longer the
    primary source."""
    global _VERSION_LOGGED
    if _EMBEDDED_VERSION:
        if not _VERSION_LOGGED:
            print(f"[updater] version {_EMBEDDED_VERSION} "
                  f"(embedded via tools/embed_version.py)", flush=True)
            _VERSION_LOGGED = True
        return _EMBEDDED_VERSION
    # Fallback — file lookup, for `python main.py` runs in a fresh
    # checkout where build.bat hasn't generated _version.py yet.
